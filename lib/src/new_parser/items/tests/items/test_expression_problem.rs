        ...
        crate::ast::TopLevel::FunctionDecl(func) => {
            match &func.lambda.body.statements[0] {
                crate::ast::Statement::Expression(expr) => {
                    match expr {
                        crate::ast::Expression::UnaryExpr(crate::ast::UnaryExpr::PrimaryExpr(primary)) => {
                            // Should have 3 secondaries: .bar, arguments, .haha
                            assert_eq!(primary.secondaries.as_ref().unwrap().len(), 3,
                                "Expected 3 secondaries: .bar, arguments, .haha");

                            // The third secondary should be .haha
                            match &primary.secondaries.as_ref().unwrap()[2] {
                                crate::ast::SecondaryExpr::Dot(crate::ast::IdentOrNumber::Ident(ident)) => {
                                    assert_eq!(ident.name, "haha", ".haha should be outside the lambda body");
                                }
