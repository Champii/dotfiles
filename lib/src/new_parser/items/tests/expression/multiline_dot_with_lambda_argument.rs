...
fn multiline_dot_with_lambda_argument() {
    ...
    assert!(result.is_ok(), "Failed to parse multiline dot with lambda: {:?}", result.err());
}
...
fn multiline_dot_chain_with_lambda_arguments() {
    ...
    // This should parse successfully
    assert!(result.is_ok(), "Failed to parse multiline dot chain with lambdas: {:?}", result.err());
}
...
fn multiline_dot_with_nested_multiline_in_lambda() {
    ...
    // This should parse successfully
    assert!(result.is_ok(), "Failed to parse multiline dot with nested multiline in lambda: {:?}", result.err());
}
...
fn multiline_dot_with_inline_lambda_then_multiline_dot() {
    ...
    // This should parse successfully
    assert!(result.is_ok(), "Failed to parse multiline dot with inline lambda then multiline dot: {:?}", result.err());
}
